...
void CImg::operator = (CImg& gray)
{
	...
	{
		m_pBMIH = (BITMAPINFOHEADER*) new BYTE[sizeof(
			BITMAPINFOHEADER) + m_nColorTableEntries * 4];
		memcpy(m_pBMIH, gray.m_pBMIH, sizeof(BITMAPINFOHEADER) +
			m_nColorTableEntries * 4);

		if (m_nColorTableEntries != 0)
			m_lpvColorTable = m_pBMIH + 1;
	}

	int nWidthBytes = WIDTHBYTES((
		m_pBMIH->biWidth) * m_pBMIH->biBitCount);
...
{
	LPBYTE*  lpData;			// 文件数据
	BITMAPINFOHEADER* pBMIH;	// 位图信息头
	LPVOID lpvColorTable = NULL;// 颜色表指针
...
BOOL CImg::SaveToFile(CFile &file)
{
	...
	bmfHeader.bfOffBits = sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER) + m_nColorTableEntries * 4;
...
BOOL CImg::Draw(CDC* pDC)
{
	...
	for (int i = 0; i < m_pBMIH->biHeight; i++)
		::SetDIBitsToDevice(*pDC, 0, 0, m_pBMIH->biWidth, m_pBMIH->biHeight, 0, 0, i, 1, m_lpData[i],
		(BITMAPINFO*)m_pBMIH, DIB_RGB_COLORS);
...
BOOL CImg::Draw(CDC* pDC, CRect rt)
{
	...
	for (int i = 0; i < m_pBMIH->biHeight; i++)
		::SetDIBitsToDevice(*pDC, rt.left, rt.top, rt.Width(), rt.Height(), rt.Width() + 10, 0, i, 1, m_lpData[i],
		(BITMAPINFO*)m_pBMIH, DIB_RGB_COLORS);

	return TRUE;
}

BOOL CImg::Draw2(CDC * pDC)
{
	if (m_pBMIH == NULL)
		return FALSE;

	for (int i = 0; i < m_pBMIH->biHeight; i++)
		::SetDIBitsToDevice(*pDC, m_pBMIH->biHeight + 30, 0, m_pBMIH->biWidth, m_pBMIH->biHeight, 0, 0, i, 1, m_lpData[i],
		(BITMAPINFO*)m_pBMIH, DIB_RGB_COLORS);

	return TRUE;
}

BOOL CImg::Draw_originalPic(CDC* pDC)
{
	if (m_pBMIH == NULL)
		return FALSE;
...
BOOL CImg::Draw_originalPic(CDC* pDC, CRect rt)
{
	if (m_pBMIH == NULL)
		return FALSE;
	int iOriginalPictureRight = 2 * m_pBMIH->biWidth + 10;

	if (rt.IsRectNull())
	{
		rt.right = iOriginalPictureRight;
		rt.bottom = m_pBMIH->biHeight;
	}
	...
	for (int i = 0; i < m_pBMIH->biHeight; i++)
		::SetDIBitsToDevice(*pDC, rt.left, rt.top, rt.Width(), rt.Height(), 0, 0, i, 1, m_lpData[i],
		(BITMAPINFO*)m_pBMIH, DIB_RGB_COLORS);
...
void CImg::Line(POINT ptStart, POINT ptEnd)
{
	CVector2D line;
	// 获取距离
	int l = (int)(sqrt((double)(ptStart.x - ptEnd.x) * (ptStart.x - ptEnd.x) + (ptStart.y - ptEnd.y) * (ptStart.y - ptEnd.y)));
...
bool CImg::Index2Gray()
{
	...
	for (i = 0; i < GetHeight(); i++)
	{
		for (int j = 0; j < GetWidthPixel(); j++)
		{
			...
			BYTE gray = (BYTE)(rgb.rgbBlue * 0.114 +
				rgb.rgbGreen * 0.587 + rgb.rgbRed * 0.299 + 0.5);
...
CImg CImg::operator - (CImg &gray)
{
	...
	int nHeight = GetHeight();			//取得图像的高和宽
	int nWidth = GetWidthPixel();
	int i, j;//循环变量
